...
<?if($arResult["CHECKED_PROPERTIES"]):?>
    <div class="catalog-controls__applied-filters applied-filters">
        ...
        <button class="applied-filters__item remove-filter-item js-reset-filters button-toggle remove_all" type="button" aria-label="Удалить фильтр" data-url="<?=$arResult["CANCEL_URLS"]['CLEAR_FILTER']?>">
...
<!--                        <span class="label">--><?//=$key?><!--:</span>-->
                        <?if($control_id != "range"):?>
                            ...
                            <button class="applied-filters__item remove-filter-item js-reset-filters button-toggle  btn-remove" data-control="<?=$control_id?>" type="button" aria-label="Удалить фильтр">
                                <span class="remove-filter-item__value button-toggle__label">
                                    <?=$key?>: <?=$value?>
                                    <span class="remove-filter-item__icon">×</span>
                                </span>
                        ...
                        <?else:?>
                            ...
                            <button class="applied-filters__item remove-filter-item js-reset-filters button-toggle" type="button" aria-label="Удалить фильтр">
                                <span class="remove-filter-item__value button-toggle__label remove_all" data-url="<?=$data_url?>">
...
<script>
    jQuery(document).on('click', ".btn-remove", function() {
        ...
        setTimeout(function() {jQuery('#set_filter').trigger('click');}, 800);
...
<!--						<div class="bx_filter_parameters_box_title" onclick="smartFilter.hideFilterProps(this)">--><?//=$arItem["NAME"]?><!--</div>-->
			...
			foreach($arResult["ITEMS"] as $key=>$arItem)
			{
				...
				<div class="bx_filter_parameters_box <?if ($arItem["DISPLAY_EXPANDED"]== "Y"):?>active<?endif?>">
					<span class="bx_filter_container_modef"></span>
<!--					<div class="bx_filter_parameters_box_title" onclick="smartFilter.hideFilterProps(this)">--><?//=$arItem["NAME"]?><!--</div>-->
					<details class="bx_filter_block main-filter__accordion accordion" <?=$arItem['DISPLAY_EXPANDED'] == 'Y' ? 'open' : ''?>>
                        <summary class="bx_filter_parameters_box_title accordion__title"><?=$arItem["NAME"]?></summary>
						...
						switch ($arItem["DISPLAY_TYPE"])
						{
								...
								<?foreach ($arItem["VALUES"] as $val => $ar):?>
									...
									<label for="<?=$ar["CONTROL_ID"]?>" data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx_filter_param_label<?=$class?>" onclick="smartFilter.keyup(BX('<?=CUtil::JSEscape($ar["CONTROL_ID"])?>')); BX.toggleClass(this, 'active');">
										...
										if ($arParams["DISPLAY_ELEMENT_COUNT"] !== "N" && isset($ar["ELEMENT_COUNT"])):
											?> <span data-role="count_<?=$ar["CONTROL_ID"]?>" class="main-filter__counter"><?= $ar["ELEMENT_COUNT"]; ?></span><?
										endif;?></span>
									</label>
								<?endforeach?>
								<?
                            ...
                            <div class="form-check">
                                ...
                                />
                                <label class="form-check-label bx_filter_param_label" for="<? echo "all_".$arCur["CONTROL_ID"] ?>">
                            ...
                            <div class="form-check">
                                ...
                                <label data-role="label_<?=$ar["CONTROL_ID"]?>" class="form-check-label bx_filter_param_label" for="<? echo $ar["CONTROL_ID"] ?>">
                                    <span class="bx_filter_param_text" title="<?=$ar["VALUE"];?>">
                                        <?=$ar["VALUE"];?>
                                    </span>
                                    <span class="main-filter__counter"></span>
                                    <? if ($arParams["DISPLAY_ELEMENT_COUNT"] !== "N" && isset($ar["ELEMENT_COUNT"])):
                                        ?> <span data-role="count_<?=$ar["CONTROL_ID"]?>" class="main-filter__counter"><? echo $ar["ELEMENT_COUNT"]; ?></span><?
                                    endif;?>
                                </label>
                            </div>
...
<!--						<input class="bx_filter_search_reset" type="submit" id="del_filter" name="del_filter" value="--><?//=GetMessage("CT_BCSF_DEL_FILTER")?><!--" />-->
                        ...
                        <button class="button w-100 bx_filter_search_button" type="submit" id="set_filter" name="set_filter" value="<?=GetMessage("CT_BCSF_SET_FILTER")?>"><?=GetMessage("CT_BCSF_SET_FILTER")?></button>
                        <br>
                        <br>
                        <button class="button w-100 bx_filter_search_reset" type="submit" id="del_filter" name="del_filter" value=""<?=GetMessage("CT_BCSF_DEL_FILTER")?>"><?=GetMessage("CT_BCSF_DEL_FILTER")?></button>

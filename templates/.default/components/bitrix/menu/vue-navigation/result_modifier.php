...
{
    $arFilter = Array("IBLOCK_ID" => "113", "ACTIVE" => "Y", "DEPTH_LEVEL" => "1");
    ...
    elseif ($obCache->StartDataCache())
    {
        if (CModule::IncludeModule("iblock"))
        {
            ...
            {
                $dbSections = CIBlockSection::GetList(array(), array("IBLOCK_ID" => "113"), false, array("ID", "SECTION_PAGE_URL", "UF_ICON", "DESCRIPTION", ));

                while($arSections = $dbSections->GetNext())
                {
                    $pictureSrc = CFile::GetFileArray($arSections["UF_ICON"]);
                    $arSectionsInfo[crc32($arSections["SECTION_PAGE_URL"])]["PICTURE"] = $pictureSrc ? $pictureSrc["SRC"] : false;
